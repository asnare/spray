...
private[http] class UriParser(input: ParserInput, charset: Charset, mode: Uri.ParsingMode) {
  ...
  def parseAbsolute(): Uri = {
    ...
    create(_scheme, _userinfo, _host, _port, collapseDotSegments(_path), _query, _fragment, input.toString)
  }
  ...
  def parseReference(): Uri = {
    ...
    create(_scheme, _userinfo, _host, _port, path, _query, _fragment, input.toString)
  }
